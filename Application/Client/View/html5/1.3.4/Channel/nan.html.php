...
<!--公告开始-->
<div class="unit">
    <div class="gongao tit" onClick="hg_gotoUrl('{:url('Book/downapp',array('sex_flag'=>'nan'),'html')}');"><p class="hidden "><span class="laba"><a href="{:url('Book/downapp',array('sex_flag'=>'nan'),'html')}"><img src="__IMG__/ic_laba.png" /></a></span>下载客户端，签到得银币，好书免费看！</p></div>
</div>
<!--公告结束-->
<!--主编推荐开始-->
<div class="unit">
    ...
    <div class="frame clearfix">
        ...
        <Hongshu:bangdan name="android_nan_zhubian" items="2">
            <li onClick="hg_gotoUrl('{:url('Book/view',array('bid'=>$row['bid']),'html')}');" class="clearfix">
                ...
                <div class="rt"><h1 class="hidden"><a href="{:url('Book/view',array('bid'=>$row['bid']),'html')}">{$row.catename}</a></h1>
                 <p>{$row.intro}</p></div>
            </li>
        </Hongshu:bangdan>
        </ul>
...
<!--今日免费开始-->
<div class="unit">
    <div class="tit"><h1>今日免费</h1><a href="{:url('Channel/free',array('sex_flag'=>'nan'),'html')}" class="more">更多></a><span class="flrt mrt10"><em class="cred" id="timer"></em></span></div>
    <div class="frame2">
        <ul id="free1">
        </ul>
    </div>
...
<script type="text/html" id="huanyihuan_tpl">
    ...
    <li onclick="hg_gotoUrl('{{ {bid:row.bid} | router:'Book/view','html'}}');" class="hidden"><span class="num num{{i+1}}">{{i+1}}</span><span class="tag">{{row.classname}} | </span>{{row.catename}}</li>
        {{/each}}
    {{/if}}
</script>
