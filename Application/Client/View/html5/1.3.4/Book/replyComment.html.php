...
<div class="unit">
    <div class="frame10 comments">
        <ul>
            <li >
                <div class="lf"><if condition="$commentinfo.avatar"><img src="{$commentinfo.avatar}" /><else/><img src="__IMG__/avater.jpg" /></if></div>
                <div class="rt">
...
<div class="pinglun">
    <div class="plcon">
    <div class="lf"></div>
        <div class="center center2"><input id="text2" class="pltext radius4" placeholder="说说你的看法" /></div>
        <div class="rt"><button class="plbtn radius2" disabled="disabled" id="tijiao2">回复</button></div>
    </div>
</div>
<!--发表评论结束-->
</block>
...
var url = "{:url('Bookajax/replyComment', array('bid'=>$bookinfo['bid'],'comment_id'=>$commentinfo['comment_id']),'do')}";
...
var is_reload = 1;
  ...
  function submit2(){
    checknum2();
      var url = "{:url('Bookajax/addreply','','do')}";
      var content = $('#text2').val();
      var data = {
         bid:{$bookinfo['bid']|intval},
         comment_id:{$commentinfo['comment_id']|intval},
         content:content,
      }
       $.ajax({
        type:'post',
