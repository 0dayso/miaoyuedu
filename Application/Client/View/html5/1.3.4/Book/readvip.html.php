...
<!-- <div class="rdcon <if condition="$blackwhite eq 1">rdbg_black</if>" id="wz"> -->
    ...
    <p class="rdbt"><span id="shouye">
            ...
            <a href="{:url('Book/xiajia','','do')}" class="rdname hidden">{$bookinfo.catename}</a>
            <else/>
            <a href="{:url('Book/view', array('bid'=>$bookinfo['bid']),'html')}" class="rdname hidden">{$bookinfo.catename}</a>
            </if>
            <span >></span>
...
Do.ready("functions",'template',function(){
    ...
    //console.log(bid+' '+chpid+' '+cidx+' '+isvip);
