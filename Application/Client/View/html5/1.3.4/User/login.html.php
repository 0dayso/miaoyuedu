...
<div class="otherlogin mtop40 clearfix">
    ...
    <ul>
        <li onClick="hg_gotoUrl('{:url('User/thirdlogin', array('type'=>'wechat','fu'=>$M_forward),'do')}');"><span><img src="__IMG__/ic_weixin2.png"/></span><p>微信登录</p></li>
...
<div class="login login3 clearfix" style="margin-bottom: 50px;">
    <form>
        <p><button type="button" class="radius4" onClick="hg_gotoUrl('{:url('User/login', array('type'=>'id','fu'=>$M_forward,'sex_flag'=>$sex_flag),'do')}')">用手机号或{:C('SITECONFIG.SITE_NAME')}账号登录</button></p>
        <!-- <p><a onClick="hg_gotoUrl('{:url('User/register',array('sex_flag'=>$sex_flag))}')" class="phbtn flrt">手机号注册</a></p> -->
       <p class=" cgray mtop20">登录即代表同意<span class="cred" onClick="hg_gotoUrl('{:url('Help/article',array('article_id'=>C('CLIENT.'.CLIENT_NAME.'.helpid'),'sex_flag'=>$sex_flag),'html')}')">{:C('SITECONFIG.SITE_NAME')}服务条款</span></p>
    </form>
</div>
<!--登录结束-->
</block>
