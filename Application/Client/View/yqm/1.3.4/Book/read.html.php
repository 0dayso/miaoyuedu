...
<block name="body">
<div class="login_tc" id="login_tc" style="display:none;">
    <div class="">
        ...
        <div class="read">
            ...
            <div class="readbom" style="display:block;" >
                ...
                <div class="dashang" id="dashang" style="display:none;">
                    ...
                    <h1 class="top20" id="ds_name">元气弹 x <span class="cb" id="shuliang">5</span></h1>
            ...
            <div class="zhongfudong empty">
                ...
                <button id="senddanmu" num="0" class="fang_rd"><img src="__IMG__/tucao.png">发送弹幕</button>
            </div>
...
function addBarrage(str, id){
    ...
    item.left = Math.floor(Math.random() * 50 + (parseInt(__barrager_times/3) - 1) * 420);
    ...
    UserManager.addListener(function(user){
        ...
        if(user.islogin){
            ...
            $('#sc').on('click',function(){
                hg_Toast('请先登录！');$('.login_tc').show();$('#input_pwd').focus();
            });
            $('#senddanmu').on('click',function(){
               hg_Toast('请先登录！');$('.login_tc').show();$('#input_pwd').focus();
            });
...
function getchp(){
 ...
 Do.ready('functions','template', function () {
        ...
        $('.read_shezhi_r a').on('click',function(){
            $('.read_shezhi_r a').removeClass('active');
        ...
        changemode(mode);
        //获取打赏道具
        var url="{:url('Bookajax/getProList',array(),'do')}";
        $.ajax({
        type:'get',
        url: url,
        //data:data,
        success: function (data){
            var name=data['prolist'][0]['name'];
            var price=data['prolist'][0]['price'];
            var jine=price*5;
            /*$('#ds_name').html(name+' x5');
            $('#jine').html(jine+'元气币');*/
        }
    });
    });
