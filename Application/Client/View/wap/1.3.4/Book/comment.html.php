...
<script type="text/html" id="huanyihuan_tpl">
    {{if list}}
        {{each list as row i}}
            ...
            <li>
                <div>
                    <h4 onClick="hg_gotoUrl('{{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment','do'}}')"><a href="{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment','do'}">{{row.nickname}}<if condition="$row.highlight_flag eq 1"><span class="ic_jh">精华</span></if><if condition="$row.lcomment eq 1"><span class="ic_cp">长评</span></if></a></h4>
                     <p onClick="hg_gotoUrl('{{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment','do'}}')"><a href="{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment','do'}">{{#row.content}}</a></p>
                     <p class="combom"><span class="fllf">{{row.time}}</span><a class="detail btn" id="zan_{{row.comment_id}}" data-id="{{row.comment_id}}">赞(<span>{{row.zan_amount}}</span>)</a><a href="{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment','do'}" onClick="hg_gotoUrl('{{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment','do'}}')" class="reply">回复({{row.reply_amount}})</a></p>
                </div>
            </li>
            {{/if}}
        {{/each}}
