    ...
    <div class="frame frame0">
        <ul id="lists">
            ...
            <li>
                ...
                <a href="{:url('Book/read', array('bid'=>$row['bid'], 'chpid'=>$row[last_readchpid]))}">
                <div onClick="hg_gotoUrl('{:url('Book/read', array('bid'=>$row['bid'], 'chpid'=>$row[last_readchpid]))}')" class="rt3">
                <else/>
                <a href="{:url('Book/readVip', array('bid'=>$row['bid'], 'chpid'=>$row[last_readchpid]), 'do')}">
...
<script type="text/html" id="huanyihuan_tpl">
    {{if list}}
        {{each list as row i}}
           <li>{{if row.isvip == 0}}
                <a href="{ {bid:row.bid,chpid:row.last_readchpid} | router:'Book/read','html'}"><div class="lf" onClick="hg_gotoUrl('{{ {bid:row.bid,chpid:row.last_readchpid} | router:'Book/read','html'}}')">{{else}}
                <a href="{ {bid:row.bid,chpid:row.last_readchpid} | router:'Book/readvip','do'}"><div class="lf" onClick="hg_gotoUrl('{{ {bid:row.bid,chpid:row.last_readchpid} | router:'Book/readvip','do'}}')">{{/if}}
