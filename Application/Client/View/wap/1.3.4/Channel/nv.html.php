...
<block name="body">
<!--公告开始-->
<Hongshu:bangdan name="static_g_indexlabanv">
{$row}
</Hongshu:bangdan>
<!--公告结束-->
<!--主编推荐开始-->
...
<!--今日免费开始-->
<div class="unit">
    <div class="tit borderbom"><h1>本期免费</h1><span class="cgray mlf10">还剩</span><span class="cred mlf10" id="timer"></span><a href="{:url('Channel/free',array('sex_flag'=>'nv'),'html')}" class="more">更多></a></div>
    <div class="frame3">
        <ul id="free1">
        </ul>
    </div>
...
<!--重磅推荐开始-->
<div class="unit">
    <div class="tit borderbom"><h1>重磅推荐</h1></div>
    <div class="frame3">
        <ul>
...
<!--畅销榜开始-->
<div class="unit">
    <div class="tit borderbom"><h1>女生小说人气榜</h1></div>
    <div class="frame3">
        <ul id="lists">
        </ul>
    </div>
...
<script type="text/html" id="huanyihuan_tpl">
    ...
    <a href="{{ {bid:row.bid} | router:'Book/view','html'}}"><li class="hidden" onClick="hg_gotoUrl('{{ {bid:row.bid} | router:'Book/view','html'}}')"><span class="num num{{i+1}}">{{i+1}}</span>{{row.catename}}<span class="zzm">{{row.author}}</span><span class="tag">{{row.classname}}</span></li></a>
        {{/each}}
    {{/if}}
</script>
<script type="text/html" id="tpl1">
{{each books as row i}}
    <a href="{{ {bid:row.bid} | router:'Book/view','html'}}"><li onClick="hg_gotoUrl('{{ {bid:row.bid} | router:'Book/view','html'}}')" class="hidden">{{row.catename}}<span class="zzm">{{row.author}}</span><span class="tag">{{row.classname}}</span>
    </li></a>
{{/each}}    
</script>
<script type="text/javascript">
