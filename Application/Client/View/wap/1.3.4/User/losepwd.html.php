...
<div class="login login2">
        ...
        <p class="yz" style="display:none;" id="sbm"><input type="text" placeholder="请输入识别码" class="radius4" id="input_sbm" /><span class="correct" id="c2"></span><a class="yzmimg radius4"><img src="{:url('User/imgcode','','do')}" lazy='y' id='imgcode' onclick='return refreshimg();'/></a></p>
        ...
        <p id="button3" style="display:none;"><button id="zc" type="button" disabled="disabled" class="disable radius4" onclick="login();">登录</button></p>
</div>
</block>
<block name="script">
...
function checksbm(){
   ...
   var url="{:url('Userajax/checkimgcode','','do')}";
   $.ajax({
        url: url,
        type: "POST",
        data: {sbm:sbm},
...
function register(obj){
    ...
    url="{:url('Userajax/verifyCode','','do')}";
    $.ajax({
        url: url,
        type: "POST",
        data: {mobileId:userName,sbm:sbm,type:'losepwd'},
        dataType: 'json',
        success: function(data){
            if(data.status == 1){
               hg_Toast(data.message);
...
function login(obj){
    ...
    lockbutton(obj);
    url="{:url('Userajax/losepwdlogin','','do')}";
    $.ajax({
        url: url,
        type: "POST",
        data: {mobileId:userName,password:pwd,sbm:sbm,type:'losepwd'},
        dataType: 'json',
        complete:function(){
           unlockbutton(obj);
        },
