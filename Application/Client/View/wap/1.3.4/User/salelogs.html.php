...
Do.ready('lazyload','functions','template',function(){
    ...
    document.onscroll = function(){
        var footHeight = $('.footer').height();
           ...
           if((iScroll + $(window).height()-30)>$('.footer').offset().top){
               LoadMore(2);
               changestatusnum();
            }
    }
...
function LoadMore(type){
    ...
    var url = "{:url('Userajax/salelogs','','do')}";
    ...
    loadMore(url,'lists','huanyihuan_tpl',fangshi,type,para, {_callback:changestatusnum});
}
</script>
</block>