...
<!--发表评论开始-->
    ...
    <div class="commentbd clearfix">
        ...
        <if condition="session('islogin')"><button type="button" class="radius4" id="tijiao" onclick="submit();">发表评论</button><else/><button type="button" class="radius4" onclick="go_url('User/login')">登录</button></if>
        <!-- </form> -->
    </div>
    ...
    {{if list}}
        {{each list as row i}}
            ...
            <li>
                <div class="lf"><img src="__IMG__/avater.jpg" /></div>
                <div class="rt">
                    <h4 onClick="go_replycomment('{{row.comment_id}}','{{row.bid}}')">{{row.nickname}}<if condition="$row.highlight_flag eq 1"><span class="ic_jh">精华</span></if><if condition="$row.lcomment eq 1"><span class="ic_cp">长评</span></if></h4>
                     <p onClick="go_replycomment('{{row.comment_id}}','{{row.bid}}')">{{#row.content}}</p>
                     <p class="combom"><span class="fllf">{{row.time}}</span><a class="detail btn" id="zan_{{row.comment_id}}" data-id="{{row.comment_id}}">赞(<span>{{row.zan_amount}}</span>)</a><a onClick="go_replycomment('{{row.comment_id}}','{{row.bid}}')" class="reply">回复({{row.reply_amount}})</a></p>
                </div>
            </li>
            {{/if}}
        {{/each}}
...
var is_reload = 1;
  ...
  function go_replycomment(cid,bid){
       var url=parseUrl({bid:bid,comment_id:cid},'Book/replyComment','open//','html');
       doClient(url);
    }
