...
<div  class="unit mtop10">
    <div class="tit7">
        <if condition="$userinfo['islogin']">余额：<span id="money">{$userinfo.money}</span>{:C('SITECONFIG.MONEY_NAME')}<span id="egold">{$userinfo.egold}</span>{:C('SITECONFIG.EMONEY_NAME')}<a onclick="go_url('Pay/index');" class="cred mlf10">充值</a>
        <else/>
        <a onclick="go_url('User/login');" class="cred mlf10">登录</a>
        </if>
    </div>
...
<div id="dstk1" class="dstk" style="display:none;">
    <div class="dstkcon">
        ...
        <div class="commentbd mtop10 clearfix">
                <p><if condition="session('islogin')"><span class="flrt cred">{$userinfo.nickname}</span><else/><a href="javascript:void(0);" onclick="go_url('User/login');" class="flrt cred">登录</a></if></p>
                ...
                <button type="button" class="radius4 <if condition='$userinfo[islogin]'>active<else/>disable</if>" id="tijiao">确认</button>
        </div>
...
<div id="dstk2" class="dstk" style="display:none;">
    <div class="dstkcon">
        <div class="dstkcontop">
            ...
            <div id="b1" class="dstkcontop02 mtop10"><h1><span id="name2">鲜花</span>×<span id="num2">0</h1><p>你还剩<span id="xh"></span></p></div>
        ...
        <div class="commentbd mtop10 clearfix">
                <button type="button" class="radius4" id="fabiao"">确认</button>
        </div>
...
function change1(type) {
    ...
    if (type == 1) {
        ...
        if (fangshi == "鲜花") {
            ...
            var shengyu=(shuliang-num-1)+'朵鲜花';
        } else {
            var shuliang = user.red;
            var shengyu=(shuliang-num-1)+'张红票';
        }
       if (num < shuliang - 1) {
                num++;
                $('#dstk2').find('.reduce').removeClass('dis');
