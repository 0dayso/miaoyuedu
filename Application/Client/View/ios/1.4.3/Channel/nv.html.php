...
<block name="banner">
    <Hongshu:bangdan name="static_ios_ad_nv2">
        <div class="banner">
            {$row}
        </div>
    </Hongshu:bangdan>
...
<!--公告开始-->
<div class="unit">
    <div class="gongao tit" onClick="go_url('User/qiandao');"><p class="hidden "><span class="laba"><img src="__IMG__/ic_laba.png" /></span>每日签到，{:C('SITECONFIG.EMONEY_NAME')}免费得！</p></div>
</div>
<!--公告结束-->
<!--主编推荐开始-->
<div class="unit">
    ...
    <div class="frame clearfix">
        <ul>
            ...
            <li class="clearfix" onClick="go_bookview('{$row['bid']}')">
...
<!--今日免费开始-->
<div class="unit">
    <div class="tit"><h1>今日免费</h1><a onclick="go_url('Channel/free');" class="more">更多></a><span class="flrt mrt10"><em class="cred" id="timer"></em></span></div>
    <div class="frame2">
        <ul id="free1">
        </ul>
    </div>
    ...
    <div class="frame4  clearfix">
        <ul>
         <foreach name="category[subclass]" item="row">
            <li class="tag2" onClick="go_search('{$row['classid']}','nv')">{$row.title}</li>
        </foreach>
        </ul>
    </div>
</div>
...
<div class="unit">
    <div class="frame14 twobk clearfix">
        <ul>
            <li onClick="go_url('Channel/xinshu');"><h2 class="cpink">女频新书</h2><p>总裁求交往</p></li>
            <li onClick="go_url('Channel/tejia');"><h2 class="cyellow">天天特价</h2><p>1{:C('SITECONFIG.MONEY_NAME')}起</p></li>
        </ul>
    </div>
</div>
<!--新书和免费结束-->
...
<!--畅销榜结束-->
<div class="unit">
    <div class="like mbom40"> <a href="javascript:change_like('nan');"  class="flrt" ><span class="ic_set  fllf" ><img src="__IMG__/ic_set.png" /></span>我喜欢看男生小说></a></div>
</div>
</block>
<block name="script">
...
<script type="text/html" id="huanyihuan_tpl">
    ...
    <li class="hidden" onClick="go_bookview('{{row.bid}}')"><span class="num num{{i+1}}">{{i+1}}</span><span class="tag">{{row.classname}} | </span>{{row.catename}}</li>
        {{/each}}
    {{/if}}
</script>
...
function go_search(classid,sex){
   var url=parseUrl({classid:classid,sex_flag:sex},'Channel/search','open//','do');
   doClient(url);
}
