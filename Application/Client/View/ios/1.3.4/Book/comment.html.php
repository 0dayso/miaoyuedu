...
<!--发表评论开始-->
    ...
    <div class="commentbd clearfix">
        ...
        <button type="button" class="disable radius4" disabled="disabled" id="tijiao" onclick="submit();">发表评论</button>
        <!-- </form> -->
    </div>
    ...
    {{if list}}
        {{each list as row i}}
            ...
            <li>
                <div class="lf"><img src="__IMG__/avater.jpg" /></div>
                <div class="rt">
                    <h4 onClick="hg_gotoUrl('{{ {comment_id:row.comment_id,bid:row.bid} | router:'Book/replyComment'}}')">{{row.nickname}}<if condition="$row.highlight_flag eq 1"><span class="ic_jh">精华</span></if><if condition="$row.lcomment eq 1"><span class="ic_cp">长评</span></if></h4>
