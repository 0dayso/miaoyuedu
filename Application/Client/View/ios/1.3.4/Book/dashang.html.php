...
<div  class="unit mtop10">
    <div class="tit7">
        <if condition="$userinfo['islogin']">余额：<span id="money">{$userinfo.money}</span>{:C('SITECONFIG.MONEY_NAME')}<span id="egold">{$userinfo.egold}</span>{:C('SITECONFIG.EMONEY_NAME')}<a href="javascript:hg_gotoUrl('{:url('Pay/index')}');" class="cred mlf10">充值</a>
        <else/>
...
<div id="dstk1" class="dstk" style="display:none;">
    <div class="dstkcon">
        ...
        <div class="commentbd mtop10 clearfix">
                <p><if condition="session('islogin')"><span class="flrt cred">{$userinfo.nickname}</span><else/><a href="javascript:void(0);" onClick="hg_gotoUrl('{:url('User/login')}')" class="flrt cred">登录</a></if></p>
