...
class PayController extends Controller {
    ...
    public function _index() {
        ...
        if (CLIENT_NAME === 'html5') {
            ...
            $pay_sort = array('ALIPAY_WAP', 'WEIXINPAY_QRCODE');
        }
        $paylists = array();
        if($pay_sort) {
...
//            if(session('uid')==3901185) {
//                $this->display('newpay');
//            } else{
//                header('location:'.url('Pay/oldpay'));
//            }
//        }
        $this->display('newpay');
    }
    ...
    public function _AlipaySDK_android() {
        ...
        if (!$out_trade_no) {
            ...
            if(session('uid')==3901185) {
                $alipay_config = array_merge($alipay_config, array(
                    'partner'=> "2088621163543742",	//合作身份者ID，以2088开头的16位纯数字
                    'key'=> "mcgugdx090pgach6dg092uanrdudwff7",	//安全检验码，以数字和字母组成的32位字符
                    'seller_email'=> "guanrong@hongshu.com",	//签约支付宝账号或卖家支付宝帐户
                    'notify_url'=> "https://wallet.hongshu.com/wapalipay/alipaywapnotifyurl.do",	//异步返回消息通知页面，用于告知商户订单状态
                    'call_back_url'=> "https://wallet.hongshu.com/wapalipay/alipaywapreturnurl.do",	//同步返回消息通知页面，用于提示商户订单状态
                ));
            }
            $amount = $amount . '.00';
