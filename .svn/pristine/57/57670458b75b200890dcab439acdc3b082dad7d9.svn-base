...
document.writeln(str);
    ...
    <div class="main1200">
        <div class="read">
            ...
            <h1 class="c3 top80">剧终。感谢观赏</h1>
            <div class="rd_xx c6 top20 bom150">
                <p>您已读完整本书了</p>
                <p>您可以 <a class="cb" href="{:url('Book/comment',array('bid'=>$bookinfo['bid']),'html')}">发发吐槽</a>，鼓励一下作者 <a class="cb">{$bookinfo.author}</a></p>
                <p>已有<span class="cb">{$bookinfo.fansnum}</span>粉丝进行了打赏</p>
                <div class="rdlast_h"></div>
            </div>
            <else/>
            <h1 class="c3 top80">待续...</h1>
            <div class="rd_xx c6 top20 bom150">
               <p>您可以 <a class="cb" href="{:url('Book/comment',array('bid'=>$bookinfo['bid']),'html')}">发发吐槽</a>，鼓励一下作者 <a class="cb">{$bookinfo.author}</a></p>
                <p>已有<span class="cb">{$bookinfo.fansnum}</span>粉丝进行了打赏</p>
                <div class="rdlast_h"></div>
            ...
            <div class="readbom" style="display:block;">
                <div class="readtextbtn" id="zhangjieinfo">
                    ...
                    <if condition="$bookinfo['prev_chpid'] eq ''">
                    <li><a class="gray readtextbtnlast readtextbtnlast_01">没有了</a></li>
                <else/>
                    <li><a href="{:url('Book/read',array('bid'=>$bookinfo['bid'],'chpid'=>$bookinfo['prev_chpid']))}" class="readtextbtnlast readtextbtnlast_01">上一章</a></li>
                </if>
            ...
            <div class="read_shezhi_hei shezhi" style="display:none;">
                    ...
                    <div class="read_shezhi_r flrt">
                        <a class="cf" size="14">Aa</a>
                        <a class="cf" size="16" style="font-size: 16px;">Aa</a>
                        <a class="cf" size="20" style="font-size: 20px;">Aa</a>
                    </div>
                </div>
            ...
            <if condition="$bookinfo['prev_chpid'] eq ''">
                ...
                <else/>
                    <button class="shangyizhang" onclick="hg_gotoUrl('{:url('Book/read',array('bid'=>$bookinfo['bid'],'chpid'=>$bookinfo['prev_chpid']))}');"><img src="__IMG__/shangyizhang.png"><a href="{:url('Book/read',array('bid'=>$bookinfo['bid'],'chpid'=>$bookinfo['prev_chpid']))}">上一章</a></button>
                </if>
                <if condition="$bookinfo['next_chpid'] eq ''">
                    <button class="xiayizhang gray"><img src="__IMG__/xiayizhang.png">没有了</button>
                <else/>
                    <button class="xiayizhang" onclick="hg_gotoUrl('{:url('Book/read',array('bid'=>$bookinfo['bid'],'chpid'=>$bookinfo['next_chpid']))}');"><img src="__IMG__/xiayizhang.png"><a href="{:url('Book/read',array('bid'=>$bookinfo['bid'],'chpid'=>$bookinfo['next_chpid']))}">下一章</a></button>
                </if>
            </div>
        </div>
...
/*function savecookie(){
    ...
    var url = "{:url('Bookajax/updateReadLog',array(),'do')}";
    $.ajax({
        type:'get',
        url: url,
        data:data,
...
function gundong(){
    ...
    if(read_height < zfd_height+pos.top){
        pos.top = read_height - zfd_height-150;
    }
    $('.zuofudong').css(pos);
