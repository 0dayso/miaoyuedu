...
<!--今日免费开始-->
<div class="unit">
    <div class="tit"><h1>今日免费</h1><a href="javascript:hg_gotoUrl('{:url('Channel/free',array('sex_flag'=>$sex_flag))}');" class="more">更多></a><span class="flrt mrt10"><em class="cred" id="timer"></em></span></div>
    <div class="frame2">
        <ul>
...
<if condition="$sex_flag eq 'nan'">
        <foreach name="category" item="row">
            <li class="tag3" onClick="hg_gotoUrl('{:url('Channel/search',array('classid'=>$row['classid'],'sex_flag'=>$sex_flag))}')">{$row.title}</li>
        </foreach>
<else/>
         <foreach name="category[subclass]" item="row">
            <li class="tag2" onClick="hg_gotoUrl('{:url('Channel/search',array('classid'=>$row['classid'],'sex_flag'=>$sex_flag))}')">{$row.title}</li>
        </foreach>
</if>
        </ul>
    </div>
...
<div class="unit">
    <div class="frame14 twobk clearfix">
        <ul>
        <if condition="$sex_flag eq 'nan'">
            <li onClick="hg_gotoUrl('{:url('Channel/xinshu',array('sex_flag'=>$sex_flag))}');"><h2 class="cgreen">男频新书</h2><p>最新免费小说免费看</p></li>
            <li onClick="hg_gotoUrl('{:url('Channel/tejia',array('sex_flag'=>$sex_flag))}');"><h2 class="cyellow">天天特价</h2><p>热销小说免费</p></li>
        <else/>
            <li onClick="hg_gotoUrl('{:url('Channel/xinshu',array('sex_flag'=>$sex_flag))}');"><h2 class="cpink">女频新书</h2><p>总裁求交往</p></li>
            <li onClick="hg_gotoUrl('{:url('Channel/tejia',array('sex_flag'=>$sex_flag))}');"><h2 class="cyellow">天天特价</h2><p>1{:C('SITECONFIG.MONEY_NAME')}起</p></li>
        </if>
        </ul>
    </div>
</div>
