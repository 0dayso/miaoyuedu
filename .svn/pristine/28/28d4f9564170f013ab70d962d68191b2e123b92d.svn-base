...
<block name="body">
<style>
        .tank{ background-color: rgba(0,0,0,.8); position: fixed;left:0;top:0;width:100%;height:100%;z-index:9999999; text-align: center;}
        .tank .like{ background: url("__IMG__/like.png") no-repeat center;  background-size: 260px;width:100%;height:150px; margin-top: 100px;}
        .tank button{width:120px;border-radius:5px;border:1px solid #fff;  background-color: transparent;height:36px;color:#fff;font-size:14px; text-align: center; line-height:32px; margin: 0 20px;}
        .tank button:hover{ background-color: #000;}
        .tank .guanbi{width:32px;height:32px; background: url("__IMG__/closebtn.png") no-repeat center; background-size: 28px;display:inline-block; position: absolute;right:20px;top:20px; }
    </style>

<!--公告开始-->
<div class="unit">
    <div class="gongao tit" onClick="hg_gotoUrl('{:url('Book/downapp',array('sex_flag'=>'nv'),'html')}');"><p class="hidden "><span class="laba"><img src="__IMG__/ic_laba.png" /></span>下载客户端，签到得银币，好书免费看！</p></div>
</div>
<!--公告结束-->
<!--主编推荐开始-->
<div class="unit">
...
<!--今日免费开始-->
<div class="unit">
    <div class="tit"><h1>今日免费</h1><a href="javascript:hg_gotoUrl('{:url('Channel/free',array('sex_flag'=>'nv'),'html')}');" class="more">更多></a><span class="flrt mrt10"><em class="cred" id="timer"></em></span></div>
    <div class="frame2">
        <ul id="free1">
        </ul>
    </div>
    ...
    <div class="frame4  clearfix">
        <ul>
         <foreach name="category[subclass]" item="row">
            <li class="tag2" onClick="hg_gotoUrl('{:url('Channel/search',array('classid'=>$row['classid'],'sex_flag'=>'nv'),'html')}')">{$row.title}</li>
        </foreach>
        </ul>
    </div>
</div>
...
<div class="unit">
    <div class="frame14 twobk clearfix">
        <ul>
            <li onClick="hg_gotoUrl('{:url('Channel/xinshu',array('sex_flag'=>'nv'),'html')}');"><h2 class="cpink">女频新书</h2><p>最新小说免费看</p></li>
            <li onClick="hg_gotoUrl('{:url('Channel/tejia',array('sex_flag'=>'nv'),'html')}');"><h2 class="cyellow">天天特价</h2><p>好书5折起</p></li>
        </ul>
    </div>
</div>
<!--新书和免费结束-->
<!--畅销榜开始-->
<div class="unit">
    <div class="tit borderbom"><h1>女生红文榜</h1></div>
    <div class="frame3">
        <ul id="lists">
        </ul>
    </div>
...
<div class="tank"><div class="like"></div> <button onClick="change_like('nan', '__ROOT__/nan.html');">男生小说</button><button onClick="change_like('nv', '__ROOT__/nv.html');">女生小说</button></div>
...
Do.ready('common','template',function(){
    ...
    if(sex && (sex=='nan' || sex=='nv')) {
        window.location.replace('/'+sex+'.html');
    }
    getfreelist();
 });
