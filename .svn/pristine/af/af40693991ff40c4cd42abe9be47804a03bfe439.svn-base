...
<div class="unit">
    ...
    <div class="frame2">
        <ul>
        <Hongshu:bangdan name="android_freenv_benjizhudai" items="3">
            <li onClick="hg_gotoUrl('{:url('Book/view',array('bid'=>$row['bid']))}')">
                <div><img data-src="{$row.face}"/>
    ...
    <div class="frame4  clearfix">
        <ul>
         <foreach name="category[subclass]" item="row">
            <li class="tag2" onClick="hg_gotoUrl('{:url('Channel/search',array('classid'=>$row['classid'],'sex_flag'=>'nv'))}')">{$row.title}</li>
        </foreach>
        </ul>
    </div>
</div>
...
<div class="unit">
    <div class="frame14 twobk clearfix">
        <ul>
            <li onClick="hg_gotoUrl('{:url('Channel/xinshu',array('sex_flag'=>'nv'))}');"><h2 class="cpink">女频新书</h2><p>总裁求交往</p></li>
            <li onClick="hg_gotoUrl('{:url('Channel/tejia',array('sex_flag'=>'nv'))}');"><h2 class="cyellow">天天特价</h2><p>1{:C('SITECONFIG.MONEY_NAME')}起</p></li>
        </ul>
    </div>
</div>
<!--新书和免费结束-->
