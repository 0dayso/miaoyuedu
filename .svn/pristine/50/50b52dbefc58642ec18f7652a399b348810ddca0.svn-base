...
<div  class="unit mtop10">
    <div class="tit7">
        <if condition="$userinfo['islogin']">{$userinfo.nickname} 余额：<span id="money">{$userinfo.money}</span>{:C('SITECONFIG.MONEY_NAME')}<span id="egold">{$userinfo.egold}</span>{:C('SITECONFIG.EMONEY_NAME')}<a onClick="hg_gotoUrl('{:url('Pay/index',array('idid'=>'usercenter'))}')" class="cred mlf10">充值</a>
        <else/>
