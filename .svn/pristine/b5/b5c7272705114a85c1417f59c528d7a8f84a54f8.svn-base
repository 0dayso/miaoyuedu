    ...
    <form>
       ...
       <h1>订阅VIP章节</h1>
        ...
        <if condition = "$need_pay">
            <span>余额不足<a class="chongzhibtn radius4" onClick="hg_gotoUrl('{:url('Pay/Index',array('idid'=>'usercenter'),'do')}')">充值</a>
        </p></if>
        <else/>
        --
        <span>请登录<a class="chongzhibtn radius4" onClick="hg_gotoUrl('{:url('User/login','','do')}')">登录</a>
        </if>
