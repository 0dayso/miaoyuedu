...
class Verify {
    ...
    function _create() {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length * $this->fontSize * 2;
        // 图片高(px)
    ...
    public function entry($id = '') {
        if ($this->animator == 1 && function_exists('imagegif')) {
            ...
            $anim = new \CMS\Gifmerge($frame, 255, 255, 255, 0, $d, $x, $y, 'C_MEMORY');
            $ht = 'gif';
            $content = $anim->getAnimation();
        } else {
            ob_start();
            $this->_create();
            $this->ttffont();
        ...
        // 保存验证码
        $code = $this->authcode(strtoupper(implode('', $this->code)));
        $secode = array();
        ...
        header("content-type: image/" . $ht);
        exit($content);
    }

    function _getTTF() {
        // 验证码使用随机字体
        $ttfPath = LIB_PATH . 'Think/Verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
    ...
    private function _background() {
        ...
        if ($this->useImgBg && $gb) {
            ...
            $colorindex = imagecolorat($bgImage, 0, 0);
            $c = imagecolorsforindex($bgImage, $colorindex);
            $colorindex = imagecolorat($bgImage, 1, 0);
            imagesetpixel($bgImage, 0, 0, $colorindex);
            $c[0] = $c['red'];
            $c[1] = $c['green'];
            $c[2] = $c['blue'];
            ...
            for ($i = 0; $i < 3; $i++) {
                ...
                $step[$i] = ($end[$i] - $start[$i]) / $this->imageW;
                $c[$i] = $start[$i];
            }
            for ($i = 0; $i < $this->imageW; $i++) {
                $color = imagecolorallocate($this->_image, $c[0], $c[1], $c[2]);
                imageline($this->_image, $i, 0, $i, $this->imageH, $color);
                $c[0] += $step[0];
                $c[1] += $step[1];
                $c[2] += $step[2];
            }
    ...
    function ttffont() {
        ...
        for ($i = 0; $i < $this->length; $i++) {
            $rand = mt_rand(0, mb_strlen($codeSet, 'utf-8') - 1);
            $code[$i] = mb_substr($codeSet, $rand, 1, 'utf-8');
            if ($this->angle) {
                $font[$i]['angle'] = mt_rand(-30, 30);
            } else {
                $font[$i]['angle'] = 0;
            }
            $box = imagettfbbox($this->fontSize, $font[$i]['angle'], $this->fontttf, $code[$i]);
            $font[$i]['width'] = (max($box[2], $box[4]) - min($box[0], $box[6])) + mt_rand(0, $this->imageW / 8);
            $font[$i]['width'] = $font[$i]['width'] > $this->imageW / $this->length ? $this->imageW / $this->length : $font[$i]['width'];
            $widthtotal += $font[$i]['width'];
        }
        $x1 = $font[0]['angle'] > 0 ? cos(deg2rad(90 - $font[0]['angle'])) * $font[0]['zheight'] : 1;
        $x2 = $this->imageW - $widthtotal;
        $x = mt_rand(min($x1, $x2), max($x1, $x2));
        $text_color = imagecolorallocate($this->_image, $this->_color[0], $this->_color[1], $this->_color[2]);
        for ($i = 0; $i < $this->length; $i++) {
            $the_box = $this->calculateTextBox($code[$i], $this->fontttf, $this->fontSize, $font[$i]['angle']);
            //TODO 目前只考虑居中显示，后面再继续完善
            imagettftext($this->_image, $this->fontSize, $font[$i]['angle'], max($x, 1), $the_box["top"] + ($this->imageH / 2) - ($the_box["height"] / 2), $text_color, $this->fontttf, $code[$i]);
            $x += $font[$i]['width'];
            if ($this->color) {
                $new_color = $text_color;
                for ($c = 0; $c < 120; $c++) {
                    $new_color = array(mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
                    $t_color = imagecolorallocate($this->_image, $new_color[0], $new_color[1], $new_color[2]);
                    if ($t_color && $new_color != $fontcolor)
                        break;
                }
                $text_color = $t_color;
                //pre($new_color);;
        ...
        $this->warping && $this->warping();
        $this->code = $code;
    }

}

function dmt_rand($a, $b) {
    $a = abs($a);
    $b = abs($b);
    $c = max($a, $b);
    $d = min($a, $b);
