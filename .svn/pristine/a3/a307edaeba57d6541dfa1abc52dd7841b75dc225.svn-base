...
<div class="readtit" id="mianbaoxie" style="width:100%;">
    <div class="readnav hidden ">
        <a href="{:url('Channel/index')}">首页</a>&gt;<a href="{:url('Book/view', array('bid'=>$bookinfo['bid']),'html')}" id="bookname">{$bookinfo.catename}</a>&gt;<a>{$chapterinfo.title}</a>
    </div>
    <div class="readset">
        <span>
            <a href="{:url('Book/chapterlist',array('bid'=>$bookinfo['bid']),'do')}" id="mulu"><img src="__IMG__/ic_list_grey.png">目录</a>
        </span>
    </div>
</div>
<!--收藏本书开始-->
