...
<div class="czway">
    ...
    <div class="methods clearfix">
        <ul>
            <li>
                <div class="way radius active" id="zfbid" zhekou="{:getPayConfig('ALIPAY')}" paytype='ALIPAY_SDK'>
            ...
            <li >
                <div class="way radius4" id="wxid" zhekou="{:getPayConfig('WEIXINPAY')}" paytype='WEIXIN_SDK'>
    ...
    <form method="POST" target="_self" action="{:url('Pay/Ipaynow/ipaynowwap')}" id="payform" style="display:none">
...
var cur_paytype = 'ALIPAY_SDK';//当前选中充值类别
...
function pay_next_step(){
    ...
    if(cur_paytype == 'WEIXINPAY'){
        $('#payform').attr('action', '__PAYDOMAIN__/weixinpay/wechatwap.do');
    } else if (cur_paytype == 'WEIXINPAY_QRCODE') {
        $('#payform').attr('action', '__PAYDOMAIN__/weixinpay/wechatwapqrcode.do');
    }
