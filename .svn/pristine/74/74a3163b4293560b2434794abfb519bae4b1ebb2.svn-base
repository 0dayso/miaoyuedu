...
class CheckEnvBehavior extends Behavior {
    ...
    public function run(&$content) {
        ...
        C($tk . '.__JS__', $static . '/Public/Client/' . $client . '/' . $version . '/js');
        C($tk . '.__CSS__', $static . '/Public/Client/' . $client . '/' . $version . '/css');
        C($tk . '.__IMG__', $static . '/Public/Client/' . $client . '/' . $version . '/images');
        C($tk . '.__ROOT__', ($_SERVER['HTTPS'] == 'on' ? 'https' : 'http') . '://' . C('CLIENT.' . $client . '.domain'));
        C('DEFAULT_V_LAYER', '../Client/View/'.$client.'/' . $version);
        ...
        //加载客户端的一些自定义配置
        $file = APP_PATH . '/Client/Conf/' . CLIENT_NAME . '.php';
        if (file_exists($file)) {
            C(include $file);
        }
        //加载客户端当前版本的一些自定义配置
        $file = APP_PATH . '/Client/Conf/' . CLIENT_NAME . '.' . CLIENT_VERSION . '.php';
        if (file_exists($file)) {
            C(include $file);
        }
    }
